...
import { useCapabilities, useGrantPositionCapability } from '@/lib/api/hooks/use-admin-access';
import { useDepartmentTree } from '@/lib/api/hooks/use-organization';
import { localizeName } from '@/lib/utils/localize';
import { needsDepartment } from '@/lib/utils/admin-utils';

interface GrantCapabilityDialogProps {
  positionPublicId: string;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function GrantCapabilityDialog({ positionPublicId, open, onOpenChange }: GrantCapabilityDialogProps) {
  const t = useTranslations('admin.access.position.grant');
  const locale = useLocale();
  const { data: caps } = useCapabilities();
  const { data: departments } = useDepartmentTree();
  const grant = useGrantPositionCapability(positionPublicId);
  ...
  useEffect(() => {
    if (open) setTimeout(() => {
      setCapabilityId(''); setScopeType(1); setScopeDepartmentId('');
    }, 0);
  }, [open]);
